...
public class ServerAddition {
    public static void main(String[] args) throws IOException {
        ...
        try (ServerSocket serverSocket = new ServerSocket(portNumber)) {
            System.out.println("Serveur Addition en attente sur le port " + portNumber);
            while (true) {
                Socket clientSocket = serverSocket.accept();
                new AdditionHandler(clientSocket).start();
            }
        } catch (IOException e) {
...
class AdditionHandler extends Thread {
    private Socket clientSocket;

    public AdditionHandler(Socket socket) {
